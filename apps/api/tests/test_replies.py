...
async def test_publish_reply_falls_back_to_quote_when_native_blocked(
    ...
    class FallbackClient:
        ...
        async def post_reply(self, access_token: str, text: str, *, in_reply_to_tweet_id: str):
            ...
            raise XApiError(
                "You have not been mentioned by this Tweet author and thus are not allowed to reply",
                status_code=403,
            )
    ...
    assert fake.reply_attempts == 1
    assert fake.quote_attempts == 1


async def test_publish_reply_falls_back_on_2026_api_restriction_wording(
    client: AsyncClient, monkeypatch
):
    """Regression: new X error text must still trigger link-quote fallback."""
    from xautopilot.services.x_client import TweetPostResult, XApiError
...
async def test_publish_reply_uses_quote_when_reply_blocked(client: AsyncClient, monkeypatch):
    ...
    # Confirmed block: skip native reply and go straight to link-quote.
    monkeypatch.setattr(
        "xautopilot.services.twitter_publish_service.target_can_reply",
